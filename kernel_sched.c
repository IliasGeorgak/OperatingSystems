...
/*
   Number of yield operations required to boost all priorities
*/
#define BOOST_COUNT 5000
...
TCB* spawn_thread(PCB* pcb, void (*func)(), PTCB* ptcb)
{
	...
	/* Set the PTCB of the TCB and reverse*/
	tcb->ptcb = ptcb;
	ptcb->tcb = tcb;
	...
	tcb->priority = NO_OF_QUEUES/2; /* The new thread is added in the middle queue */
...
rlnode SCHED[NO_OF_QUEUES]; /* The scheduler queue */
rlnode TIMEOUT_LIST; /* The list of threads with a timeout */
Mutex sched_spinlock = MUTEX_INIT; /* spinlock for scheduler queue */
int yield_calls = 0; /* Counter for determining when the scheduler should boost the priority of all threads*/
...
{
	/* Insert at the end of the scheduling list with current thread's priority*/
...
static TCB* sched_queue_select(TCB* current)
{
    ...
    int highestPriorityList = 0;

    while(is_rlist_empty(&SCHED[highestPriorityList]) && highestPriorityList<=NO_OF_QUEUES-2){
        highestPriorityList++;
    }
	/* Get the head of the SCHED list with the highest priority*/
    rlnode* sel = rlist_pop_front(&SCHED[highestPriorityList]);
...
/*
Function to boost all threads priority after a certain amount of yield calls
MUST BE USED WITH THE LOCK HELD
*/
void boostAllThreads(){
    for(int i=NO_OF_QUEUES-1;i>0;i--){
        while(!is_rlist_empty(&SCHED[i])) {  /* Go through each element of this priority queue, remove it and place it in the above queue */
            rlnode* head = rlist_pop_front(&SCHED[i]);
            TCB* threadToBoost = head->tcb;
            threadToBoost->priority = threadToBoost->priority-1; /* increase priority */
            sched_queue_add(threadToBoost);
        }
    }
}
...
void yield(enum SCHED_CAUSE cause)
{
	...
	switch(cause){
		/* If the process expired its time quantum the priority decreases(highest priority is 0 so the variable INCREASES) */
		case SCHED_QUANTUM:
			if(current->priority<NO_OF_QUEUES-1){
				current->priority++;
			}
			break;
		/* If the process gave up the CPU for I/O purposes the priority increases */
		case SCHED_IO:
			if(current->priority>0){
				current->priority--;
			}
			break;
		/* If the process has called yield due to inability to aquire the lock two 
			times in a row in order to avoid a deadlock we decrease its priority */
		case SCHED_MUTEX:
...
{
    for(int i=NO_OF_QUEUES-1;i>-1;i--)
...
void run_scheduler()
{
	...
	curcore->idle_thread.priority = NO_OF_QUEUES-1; /* Lowest priority */
